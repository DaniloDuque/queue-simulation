...
public class SimulationModule extends AbstractModule {
    ...
    @Provides
    @Singleton
    PoissonDistribution providePoissonDistribution() {
        return new PoissonDistribution(Constants.ARRIVAL_LAMBDA);
    }

    @Provides
    @Singleton
    @Named(Constants.CASHIER_STATION_NAME)
    ServiceStation provideCashier() {
    ...
    EventGenerator provideEventGenerator(StationRouter stationRouter,
        ...
        return new EventGenerator(stationRouter, poissonDistribution, simulationEngine);
    }
}
