...
public class StatisticsCollector {
    ...
    public void addStationQueueTime(String stationName, double queueTime) {
        stationQueueTimes.put(stationName, stationQueueTimes.get(stationName) == null
                                        ? queueTime
                                        : stationQueueTimes.get(stationName) + queueTime);
        stationCounts.put(stationName, stationCounts.get(stationName) == null
                                        ? 1
                                        : stationCounts.get(stationName) + 1);
    }

    public void addStationServiceTime(String stationName, double serviceTime) {
        stationServiceTimes.put(stationName, stationServiceTimes.get(stationName) == null
                                        ? serviceTime
                                        : stationServiceTimes.get(stationName)
                                                                        + serviceTime);
    }
    ...
    public void printStats() {
        ...
        double[] queueTimes = servedClients.stream()
                                        .mapToDouble(Client::getTotalWaitingTimeInQueue)
                                        .toArray();
        double[] serviceTimes = servedClients.stream()
        ...
        for (String station : stationQueueTimes.keySet()) {
            ...
            double avgService = stationServiceTimes.get(station)
                                            / stationCounts.get(station);
            int throughput = stationCounts.get(station);
            log.info("{}: Queue {}, Service {}, Completed {}", station,
    ...
    private double getMedian(double[] sortedArray) {
        ...
        return n % 2 == 0
                                        ? (sortedArray[n / 2 - 1] + sortedArray[n / 2])
                                                                        / 2.0
                                        : sortedArray[n / 2];
    }
}
